...
CHECKLIST_SECTIONS = {
    "🌊 Волновой анализ": {
        ...
        "Удлинение по Фибоначчи 161-227": (10, "Подтверждение окончания тренда"),
        "Удлинение по Фибоначчи 227-261": (9, "Подтверждение окончания тренда"),
        "5 волн + наклонка + кочерга": (10, "Идеальный сетап", "⚠️ Возможны ложные пробои"),
    },
    "📊 Технический анализ": {
        "Пробитие линии тренда": (7, "Ключевой сигнал смены направления", "⚠️ Возможны ложные пробои при боковике"),
        "Движение по тренду": (7, "Следование за глобальным трендом"),
        "Движение по тренду с BTC": (4, "Альты следуют за BTC"),
        "Движение в зоны ликвидности COINGLASS": (2, "Цена стремится к стопам и ликвидациям"),
        "Объемы на текущем ТФ": (4, "Подтверждает силу движения"),
    },
    "💡 Smart Money": {
        "Сигнал от Student на 1 минутке": (4, "Подтверждение 3 волны"), 
    ...
    "🛡️ Фильтры": {
        "НЕ КИНЖАЛ": (2, "Цена НЕ должна двигаться резкими тенями без структуры"),
        "Более 1,5% TakeProfit": (3, "Цель сделки — адекватное РРМ"),
        "РММ соблюден 1:1 МИНИМУМ": (4, "Соотношение риск/прибыль не менее 1:1"),
    }
}
...
def render_checklist_entry():
    ...
    if st.button("📊 Оценить сделку"):
        ...
        with col2:
            advanced_threshold = TRADING_LEVELS["Профессиональный"]
        ...
        with col3:
            ludka_threshold = TRADING_LEVELS["Лудомания"]
        ...
        if total_score >= TRADING_LEVELS["Классика"]:
            ...
            text_color = "#FFFFFF"
            st.balloons()
        elif total_score >= TRADING_LEVELS["Лудомания"]:
        ...
        if total_score < TRADING_LEVELS["Лудомания"]:
            missing_points = TRADING_LEVELS["Лудомания"] - total_score
